...
protocol SupplierGroupRepositoryProtocol {
    ...
    func fetchAll(
        request: FetchAll,
        on db: Database
    ) async throws -> PaginatedResponse<SupplierGroup>
    
    func fetchById(
        request: GeneralRequest.FetchById,
        on db: Database
    ) async throws -> SupplierGroup
    ...
    func create(
        request: SupplierGroupRequest.Create,
        on db: Database
    ) async throws -> SupplierGroup
    
    func update(
        byId: GeneralRequest.FetchById,
        request: SupplierGroupRequest.Update,
        on db: Database
    ) async throws -> SupplierGroup
...
class SupplierGroupRepository: SupplierGroupRepositoryProtocol {
    ...
    func fetchAll(
        ...
        let query = SupplierGroup.query(on: db)
    ...
    func fetchById(
        ...
        guard
            let found = try await SupplierGroup.query(on: db).filter(\.$id == request.id).first()
        else {
    ...
    func fetchByName(
        ...
        guard
            let found = try await SupplierGroup.query(on: db).filter(\.$name == request.name).first()
        else {
            throw DefaultError.notFound
        }
    ...
    func searchByName(
        ...
        let query = SupplierGroup.query(on: db).filter(\.$name =~ regexPattern)
    ...
    func create(
        ...
        if let _ = try? await fetchByName(request: .init(name: request.name),
            ...
            let group = SupplierGroup(name: request.name,
...
private extension SupplierGroupRepository {
    func sortQuery(
        query: QueryBuilder<SupplierGroup>,
        sortBy: SortBy,
        sortOrder: SortOrder,
        page: Int,
        perPage: Int
